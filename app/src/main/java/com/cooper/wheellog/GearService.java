...
public class GearService extends SAAgent
{
    ...
    // Same as in sapservices.xml on both sides
    public final static int SAP_SERVICE_CHANNEL_ID = 142;
    ...
    void startSendingData()
    {
        if (!mIsListening)
        {
            ...
            // Location retreived each second, but data sent twice a second.
            startKeepAliveTimer();
        }
    }
    ...
    protected void onServiceConnectionResponse(SAPeerAgent agent, SASocket currentConnection, int result)
    {
        ...
        if (result == CONNECTION_SUCCESS)
        {
            if (currentConnection != null)
            {
                ...
                // We start sending when watch connects
                reevaluateNeedToSend();
            }
            else
            {
    ...
    protected void onServiceConnectionRequested(SAPeerAgent agent)
    {
        ...
        // The watch initiates always the connection
    ...
    public void onCreate()
    {
        ...
        try
        {
            // Y expect this to do nothing for non-Samsung devices
